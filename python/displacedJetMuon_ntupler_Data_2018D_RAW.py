...
process = cms.Process("displacedJetMuonNtupler",Run2_2018)
...
#process.ntupler = cms.Path(process.ntuples)
process.p = cms.Path(process.muonSystemClusterSelSeq * process.ntuples)

#Define Execution Paths
process.outputPath = cms.EndPath(process.output)
#process.p = cms.Path(process.muonCSCDigis * process.muonDTDigis * process.dt1DRecHits * process.csc2DRecHits * process.ntuples )
#process.p = cms.Path(process.muonCSCDigis * process.muonDTDigis * process.dt1DRecHits * process.csc2DRecHits * process.outputPath )
#process.p = cms.Path(process.muonCSCDigis * process.muonDTDigis *  process.dt1DRecHits * process.csc2DRecHits)
